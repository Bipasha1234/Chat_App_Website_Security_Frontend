  ...
  const highlightMatch = (text, query) => {
    ...
    return text.split(new RegExp(`(${query})`, "gi")).map((part, index) =>
      part.toLowerCase() === query.toLowerCase() ? (
        <mark key={index} className="bg-blue-300">{part}</mark>
      ) : (
        part
      )
    );
            ...
            messages.map((message) => (
              ...
              >
                ...
                <div className="text-sm">
                  <p className="font-semibold mb-2">{message.senderId?.fullName}</p>
                  {message.text && (
                    <p className="px-4 py-2 rounded-lg bg-gray-200 text-gray-800">
                ...
                <div className="flex items-center gap-3 bg-[#edf3f0] p-3 rounded-lg shadow-md mt-2 max-w-xs">
                  ...
                  <time className="text-xs opacity-60 mt-2 block text-gray-500">
