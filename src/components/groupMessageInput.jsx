    ...
    try {
      ...
      if (documentFile) {
        ...
        reader.onloadend = async () => {
          ...
          await sendGroupMessage(messageData); // Wait until upload is complete
...
{audioURL && ( 
      <div className="mb-3 flex items-center gap-2">
          ...
          <button onClick={removeAudio} className="w-5 h-5 rounded-full bg-gray-300 flex items-center justify-center" type="button" aria-label="Remove audio">
            <X className="size-3 text-red-500" />
          </button>
        </div>
      )}
      ...
      <form onSubmit={handleSendMessage} className="flex items-center gap-2">
        <div className="relative">
          ...
          {showMenu && (
            <div className="absolute bottom-16 left-0 mt-2 w-36  shadow-md rounded-md border bg-base-100 p-2 flex flex-col gap-2">
              <button className="flex items-center gap-2 p-2  rounded-md" onClick={openCamera}>
                <Camera size={20} className="text-green-500" />
                <h1>Camera</h1>
              </button>
              <button className="flex items-center gap-2 p-2  rounded-md" onClick={() =>  {setShowMenu(false) ;documentInputRef.current?.click()}}>
              <File size={20} className="text-purple-500" /> Document
            </button>
              <button className="flex items-center gap-2 p-2  rounded-md" onClick={() => {
                setShowMenu(false);
      ...
      <div className="flex-1 flex gap-2">
           <input
            type="text"
            className="py-2 px-3 rounded-lg border outline-none  shadow-sm flex-1  sm:text-md text-sm"
