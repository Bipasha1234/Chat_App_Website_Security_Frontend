...
const MessageInput = () => {
  ...
  const { sendMessage, getMessages, selectedUser, authUser } = useChatStore();
    ...
    try {
      ...
      if (documentFile) {
        ...
        reader.onloadend = async () => {
          ...
          // After sending, refetch messages so UI updates immediately
          if (selectedUser?._id) {
        ...
        <div className="fixed inset-0 bg-base-100 bg-opacity-50 flex items-center justify-center z-50">
          <div className="p-4 rounded-lg shadow-lg relative">
            ...
            <div className="flex justify-between mt-4">
              <button onClick={captureImage} className="bg-green-500 px-4 py-2 rounded-lg">Capture</button>
              <button onClick={closeCamera} className="bg-red-500 text-white px-4 py-2 rounded-lg">Close</button>
            </div>
          </div>
        </div>
      )}
